...
def predict_captions(model, dataloader, text_field):
    ...
    gen = {}
    gts = {}
    f = open("caption_results.json", "w+", encoding="utf8")
    result = []
    with tqdm(desc='Evaluation', unit='it', total=len(dataloader)) as pbar:
        for it, (images, caps_gt) in enumerate(iter(dataloader)):
            ...
            for i, (gts_i, gen_i) in enumerate(zip(caps_gt, caps_gen)):
                ...
                result.append({"Caption pred": gen_i.strip().replace('_', ' '), "Caption":gts_i})
...
if __name__ == '__main__':
    ...
    data = torch.load('./saved_models/%s.pth' % args.model, map_location=device)
