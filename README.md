...
| `--tokenize_level`| Syllable or Word tokenizer|
| `--use_pre_train`| Use pre-train word embedding (yes/no)|

For example:
To train model with pre-train word embedding Phow2v
```
python train.py --exp_name m2_word_pretrain --batch_size 50 --m 40 --head 8 --warmup 10000 --features_path /path/to/features --annotation_folder /path/to/annotations --tokenize_level word --use_pre_train yes
```
