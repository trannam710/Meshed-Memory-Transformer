...
class MeshedDecoderEmbedding(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, tokenize_level, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1,
                 self_att_module=None, enc_att_module=None, self_att_module_kwargs=None, enc_att_module_kwargs=None):
        super(MeshedDecoderEmbedding, self).__init__()
        self.d_model = d_model
        self.word_emb = nn.Embedding(vocab_size, d_model, padding_idx=padding_idx)
        self.word_emb.weight = nn.Parameter(get_embedding_matrix(tokenize_level))
